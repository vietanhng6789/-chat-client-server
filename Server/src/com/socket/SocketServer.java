...
public class SocketServer implements Runnable {
    ...
    public int clientCount = 0, port = 8899;
    public ServerFrame ui;
    public Database db;
	...
	try{  
	    ...
	    ui.jTextArea1.append("Bat Dau Ket Noi Server. IP May Tinh: " + InetAddress.getLocalHost() + ", Port: " + server.getLocalPort());
	    start(); 
        }
	catch(IOException ioe){  
            ui.jTextArea1.append("Khong The Tim Thay port : " + port + "\nRetrying"); 
            ui.RetryStart(0);
	}
    }
	...
	try{  
	    server = new ServerSocket(port);
            port = server.getLocalPort();
	    ui.jTextArea1.append("Bat Dau Ket Noi Server. IP : " + InetAddress.getLocalHost() + ", Port: " + server.getLocalPort());
	    start(); 
        }
	catch(IOException ioe){  
	...
	if (msg.content.equals(".bye")){
            ...
            if(msg.type.equals("login")){
                ...
                if(msg.recipient.equals("All")){
                    clients[findClient(ID)].send(new Message("message", "SERVER", "Tai Len 'All' Bi Cam", msg.sender));
                }
                else{
